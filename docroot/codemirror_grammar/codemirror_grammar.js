...
function preprocess_grammar( grammar )
{
    ...
    console.log('preprocess_grammar:', grammar);
    return grammar;
}
...
function preprocess_and_parse_grammar( grammar )
{
    ...
    console.log('preprocess_and_parse_grammar:', grammar);
    return grammar;
}
...
function parse_grammar( grammar )
{
    ...
    grammar.__parsed = 1;
    console.log('parse_grammar:', grammar);
    return grammar;
}
...
let tokenize_counter = 0;
function tokenize( t, stream, state, token )
{
    let id = ++tokenize_counter;
    //console.log( t );
    console.log(`tokenize(${id}) ${t.name}`);
    if ( !t ) return false;
    var T = t.type,
        t_ = T_COMPOSITE & T
        ? t_composite
    ...
    const r = t_( t, stream, state, token )
    if (r !== false)
        console.log(`tokenized(${id}) r:${r} t.name:${t.name} token.type:${token.type} token.str:"${token.str}"`);
    return r;
}
